################################################################################
#                             Corr SHAP vs. variance term
# Script for generating correlation plot betweem phenotypic variance term and
# mean |SHAP| importance measure for simulated traits. Generalized to work for
# all architecture types (1-10).
################################################################################
...
# Fit a linear model 
lm_model <- lm(variance_term_df$snp_effects ~ shap_frame_df$abs_shap)
...
cor_val <- cor(variance_term_df$snp_effects, shap_frame_df$abs_shap)
...
plot(
  shap_frame_df$abs_shap, variance_term_df$snp_effects,  
  xlab = "mean |SHAP|",  # Styled x-axis label
  ...
  xlim = range(shap_frame_df$abs_shap)
)
